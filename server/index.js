const {
  ...
  const init = async()=> {
    ...
    console.log(await fetchFavorites(moe.id));
    await destroyFavorite({ user_id: moe.id, id: favorite.id });
    console.log(await fetchFavorites(moe.id));
    ...
    console.log(`curl -X POST localhost:${port}/api/users/${moe.id}/favorites -d '{"product_id": "${foo.id}"}' -H "Content-Type:application/json" -v`);
    console.log(`curl localhost:${port}/api/users/${moe.id}/favorites -v`);
    const favoriteToDelete = await createFavorite({ user_id: lucy.id, product_id: fip.id });
  
    console.log('==== RUN TO TEST DELETE ====');
    console.log(`curl localhost:${port}/api/users/${lucy.id}/favorites`);
    console.log(`curl -X DELETE localhost:${port}/api/users/${lucy.id}/favorites/${favoriteToDelete.id} -v`);
    console.log(`curl localhost:${port}/api/users/${lucy.id}/favorites`);
